...
## ★ 대표 신상/사업 (반복 실수 금지 — 2026-08-12)
- **대표는 "법인 기사"(법인택시 일차 도급 운전직) = 직업.** 개인택시 아님. (레이더/홈 개인택시 로직과 헷갈리지 말 것.)
- **콜레이더 앱은 아직 사업자등록 없음 — 일부러 안 냄.** 나중에 국책 개발자 지원사업(국가개발자 지원)을 통해 사업 등록할 계획. → 위치기반서비스사업 신고 등 "사업자등록 필요" 절차는 그 시점 이후 단계다. 지금 "개인 사업자등록 내라/있다" 가정 금지.
- **위치정보법 이슈(2026-08-12 원스토어 심사)**: 앱이 GPS 궤적(track_points)·운행좌표를 서버 저장 → '위치 전송' 표기 시 위치기반서비스사업 신고필증 필요. '미전송' 허위표기는 법적책임+판매중지 리스크. 코드로 좌표 제거(미전송화)는 레이더 핵심 죽여서 제외. 결론=사업자등록 되는 대로 '전송'+신고(A)로 감. 그 전까지 운영 가능 여부는 원스토어·위치정보지원센터(02-588-0185) 확인 중.
...
## ★ 보류 아이디어 (급하지 않음 · 기억용)
- **레이더 지도 "네비처럼 내 위치 따라가기"** (대표: "급한 것 아님, 나중에"): 카카오 벡터맵에 현재 GPS(`LocationTrackingService.currentLat/Lng`) 커스텀 마커 + 실시간 이동표시 + 방향 화살표 + 우하단 "내 위치로" 버튼. 지도 열 때 서울시청 하드코딩(`DriverMapScreen.kt` getPosition 101줄 `LatLng.from(37.5665,126.9780)`)이 아니라 내 위치로 중심 이동. 라벨 추가는 185줄 labelManager 참고. 운행 안 할 때도 표시하려면 레이더 화면 열려 있는 동안 위치 provider 켜기(배터리 소폭).
...
- **v54 추가완료**: ④ 레이더 지도 내 위치 파란마커(e9e91d1/1479ccb, 전용레이어+2초 GPS추종, **폰 검증완료**). ⑤⑥ 안내 GPS/지역기준(서버 eac982c/클라 c828ae7): radar/personal '돈되는 목적지'를 현위치 15km 출발 기준으로 필터+개인 얇으면 전체기사 폴백(destPooled). **서버 라이브 검증완료**(강남좌표→강남발 airport목적지, 데이터없는지역→destPooled=true). 클라 음성문구도 "지금 위치에서/이 근처 전체기사 기준"으로 명확화.
- **v54 추가완료2**: 학습카드=정확도% 대신 '누적 교정 N건'(우버0원 등으로 0%가 고장처럼 보이던 것, HomeScreen L1252). 가스영수증 LPG 리터파싱(ImageImportActivity parseReceipt): **리터=소수점 정확히 3자리**(25.394/49.160)로 한정→단가(2자리)·판매시각(01:26.14)·금액(콤마)과 구분, 콤마뒤 제외(1,199.00 방지), Node로 검증(r1=25.394 r2=49.16). import/bulk가 liters 저장. **HERO(손님내리면) merge시도했다 되돌림**: 전체기사 dest-risk가 전국(제주·애월 등)이라 GPS필터 없어 오염+큰응답 항상호출시 예외로 HERO빔 → 원래 '내기록 우선/얇으면 전체폴백'으로 복구(+전체호출 개별try).
- **★ 다음 작업(대표 요청): 레이더 지역분류.** 지금 전체기사 집계가 동단위 뒤죽박죽(삼도이동·주안6동·애월읍+서초동+강남구). 대표기준: **서울=전체, 인천·경기=시별, 충청/경상/전라=남북 시별, 제주=제주시/서귀포시.** 서버 집계에서 목적지 좌표를 광역단위로 매핑(바운딩박스 or 시도 테이블) 필요. 서버만이라 앱빌드 무관.
- **v54 남은 것**: ① 홈 근무카드 위로(근무블록 300줄+ 상태·다이얼로그라 startMeter/endShiftNow/pushWorkSession/workDayKey 로직 다 엮임 → 컴포넌트 분리 리팩터 필요, blind이동은 회귀위험 커서 별도 집중세션에서. 출근버튼 로직 HomeScreen L1101-1109). 후순위: 학습정확도 채점버그(amount correct≈11/1559, 홈카드가 0%로 오해 - /api/feedback/accuracy·feedback.js), 영수증OCR 실검증+가스영수증 학습(영수증가스·가스비영수증 폴더에 대표가 10장 제공). ② 자동회전=앱무관(코드에 회전코드 0, USB/삼성 연결쪽). 미터기 소스삭제=이미 CORE_ONLY숨김+MediaProjection v30제거라 삭제 비권장.
- **v55 / 2.5.5 빌드완료 (커밋 ca20209, `_releases/v55/` 보관, BUILD SUCCESSFUL, 원스토어 업로드 예정).** 2건만 담음(안전 우선): #4 요금 총액 오긁기 방지(extractFare에 홈 대시보드 마커 '안전 도구 키트/운행 리스트/운행 명세서/운행 플래너' 추가 — 468 유저 267,519원 누적액 오긁기 케이스), #1 콜취소 후 플로팅 먹통(FloatingTripService onButtonTap: 자동트립 탭 시 무의미 토스트→armAutoCancel 무장, 취소가 길게누름에만 숨어있던 문제 해결). **다음 versionCode는 56+.**
- **v56 예정(미착수)**: #2 통행료→지출 자동분리(대표 결정: 통행 요금값>0이면 지출로, 매출은 미터만. 실기기 중복기록 테스트 필요), #3 자정 날짜귀속(businessDateFor를 출근시각→완료시각+day_start_hour 경계로. 서버+클라 동시수정, 야간기사 회귀방지 위해 day_start_hour 설정 UI 필요). 진단근거: 468 로그 오긁기 확인, business_date=work_start(출근일) 기준이라 23:50~00:06 운행이 전날로 태깅됨(대표 수동이동 확인).
- v52 = v51 전체 + #7-2 가맹 자동배차 콜카드 오탐강화 + START_SCREEN 진단로그. v51: R1 유령트립회복·우버스톨/0원·성능팩·R8난독화·지도필터·⑥궤적실차·④boarded_at·⑦콜멈춤오탐·②③거리지터·⑤플로팅숨김·①지도내위치.
