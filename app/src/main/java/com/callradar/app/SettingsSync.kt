    ...
    fun restore(ctx: Context, userId: String) {
        ...
        Thread {
            try {
                ...
                // 수수료(commission_rate)는 복원하지 않는다. 두 가지 이유가 겹친다.
                //  ① 서버엔 카카오·우버·티머니를 '합산한 값' 하나만 있어 되돌릴 수 없다
                //     (9%가 3+3+3인지 9+0+0인지 구분 불가 → 복원하면 틀린 값이 된다)
                //  ② 애초에 회사·계약마다 다른 기사 고유 영역이라, 서버가 임의로 채울 성질이 아니다.
                //  대신 가맹 형태를 고르는 자리에서 이어서 묻는다(fee_* 는 로컬 전용, 소수점 Float).
